...
def the_test(id_user, repository):
...
def index_project_files(self, id_user, repository):
    ...
    id_repository = repository.get("id")
    id_project = repository.get("projectId")
    is_github_repo = repository.get("repositoryType") == "github"

    # PULL DATA
    files = []
    if is_github_repo:
        project_name = repository.get("repositoryName") + '|' + \
            repository.get("repositoryOwner")
    else:
        project_name = id_repository
    ...
    if is_github_repo:
        ...
        files = get_unsynced_repository_docs(id_repository)
        # get files from repo
    ...
    initialization_output_logger.setFormatter(formatter)
    output_logger = logging.getLogger(project_name)
    output_logger.addHandler(initialization_output_logger)
    output_logger.info("Starting analysis of " +
                       project_name.replace('|', '/')+"...")
    ...
    config_list = get_gpt_response_from_template(
        data={'app_name': project_name, 'trimmable_content': list_of_files_concatenated}, template='what_are_these_config_files', trim_content=True)
    config_list = json.loads(config_list)
    output_logger.info("Root configuration metadata retrieved successfully.")
    ...
    output_logger.info("Process finished.")
    output_logger.removeHandler(initialization_output_logger)
    self.update_state(state='FINISHED')
