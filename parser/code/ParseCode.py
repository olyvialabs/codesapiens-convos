...
def generate_documentation_for_project_per_file(project: ProjectStructure):
    ...
    for file in all_files:
        ...
        if file_extension in temp_file_path.name == 'package.json':
            get_gpt_response_from_template(
                data=get_package_json_data(file.absolute_path), template='parse_package_json', trim_content=True, save_to_subfolder=temp_file_path.parent, save_to_name=temp_file_path.stem + '.md')
            get_gpt_response_from_template(
                data=get_package_json_data_for_dependencies(file.absolute_path), template='parse_package_json_dependencies', trim_content=True, save_to_subfolder=temp_file_path.parent, save_to_name=temp_file_path.stem + '_dependencies' + '.md')
        elif file_extension in list_of_accepted_docs_file_extensions:
            get_gpt_response_from_template(
                data={'file_name': file.name}, template='parse_document_file', trim_content=True, trim_path=file.absolute_path, save_to_subfolder=temp_file_path.parent, save_to_name=temp_file_path.stem + '.md')
        else:
            get_gpt_response_from_template(
                data={'file_name': file.name}, template='document_file_prompt', trim_content=True, trim_path=file.absolute_path, save_to_subfolder=temp_file_path.parent, save_to_name=temp_file_path.stem + '.md')
...
def generate_documentation_for_project_per_folder(project: ProjectStructure):
    all_folders = project.get_all_folders()
    for folder in all_folders:
        print(f'Now processing: ${folder.path}')
        ...
        for file in temp_project_struct.files:
            list_of_files_listed_in_folder.append(
                {'name': file.name, 'path': file.path, 'fileOrFolder': 'file', 'summary': get_file_summary(file.absolute_path)})
        for inner_folder in temp_project_struct.folders:
            list_of_files_listed_in_folder.append(
                {'name': inner_folder.name, 'path': inner_folder.path, 'fileOrFolder': 'folder', 'summary': get_folder_summary(inner_folder.absolute_path)})
        query_content = json.dumps(list_of_files_listed_in_folder)
        temp_folder_path = Path(folder.absolute_path)
